...
class TextClassificationArguments(TrainingArgs):
    ...
    launcher: str = field(
        ...
        metadata={
            'help':
            'Distributed launcher passed to DDPHook: pytorch / mpi / slurm. '
            'When using torchrun, omit this to auto-select pytorch if WORLD_SIZE>1.',
        })
    ...
    if cfg.train.lr_scheduler.type == 'LinearLR':
        cfg.train.lr_scheduler['total_iters'] = \
            int(len(train_dataset) /
                cfg.train.dataloader.batch_size_per_gpu) * cfg.train.max_epochs
    return cfg
...
_launcher = args.get('launcher')
if _launcher is not None:
    kwargs['launcher'] = _launcher  # 用于支持分布式训练
